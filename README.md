...
8 s SDRAM ring buffer feeding 8 grain voices (Hann-windowed) → texture shaper (SW1: decimator/fold, event-driven glitch zones, or ringmod) → optional Bode SSB shifter (SW2=DOWN) → K5 bipolar: Clouds reverb (CCW) or a tanh-saturated feedback loop (CW, 120 Hz HPF in the return, build-up + on-play duckers), the two mutually exclusive. SW2 picks the harmony source: fixed interval, resonance window pick, or frequency shifter. K2 at noon bypasses the grain engine and routes the dry directly into the texture shaper for a non-delay path with micro-stutter on K3.
...
| KNOB 2            | Buffer range (bipolar)      | **Noon (±6 %)** = direct-texture: grain engine bypassed, input routes straight to the texture shaper (K3 = micro-stutter). Past the pad either way = buffer depth 100 ms → 8 s + timescale; **sign sets playback direction** — CW = forward, CCW = backward. Fully CCW = deepest buffer, played backward                                                                                                                                                                                                                                                                                   |
| KNOB 3            | Character / Glitch          | **Grain mode**: CCW = long, slow smear (grains stretch ~0.3 → 2 s, overlap held so the rate falls — a granular multi-tap that leans on the deep buffer), CW = short/sharp/chaotic glitch. **Direct-texture mode** (K2 noon): micro-stutter — CCW = clean, CW = frequent choppy repeats                                                                                                                                                                                                                                                                       |
| KNOB 4            | Texture amount              | Depends on SW1 position. See Switch 1 notes                                                                                                                                                                                                                                                                                                                                                                                              |
| KNOB 5            | Reverb / Feedback (bipolar) | **CCW** = Clouds reverb amount (0→1). **Center (±5%)** = off. **CW** = ring-buffer feedback (0 → full), tanh-saturated and ducked so it self-limits into a controlled drone rather than running away. Reverb tail does not feed the ring buffer                                                                                                                                                                                                                                                                                          |
...
| KNOB 6            | Mix                    | 0 = full dry, 1 = full wet. Equal-power curve                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| SWITCH 1          | Drive                  | **UP** - Sine wavefolder (K4 CW) / Chebyshev waveshaper (K4 CCW), noon = clean<br/>**MIDDLE** - Gated bit-flipper (K4 CW, env-gated) / Tube-Screamer→tube-amp overdrive (K4 CCW), noon = clean<br/>**DOWN** - Pitch-tracked synth oscillator (K4 = saw ↔ rect timbre morph, raw-env VCA pre-filter)                                                                                                                                                                                                                                                                                                                                       |
