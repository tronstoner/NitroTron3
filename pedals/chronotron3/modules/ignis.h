...
//
// No onset/pitch analysis yet: the note set is a FIXED chord (ignis_k::CHORD_*).
...
// Reference math: saturation.py / validate.py.  All tuning in ignis_constants.h.
//
...
// ---------------------------------------------------------------------------
static float DSY_SDRAM_BSS ignis_comb_buf[ignis_k::MAX_VOICES]
                                         [ignis_k::COMB_MAX_SAMPLES];
...
class Ignis : public Module {
 public:
  ...
  void Init(float sr) override {
    sr_ = sr;
    for (int v = 0; v < ignis_k::MAX_VOICES; v++) combs_[v].Init(ignis_comb_buf[v]);
    for (int v = 0; v < ignis_k::MAX_VOICES; v++) modals_[v].Reset();
    dc_x1_ = dc_y1_ = 0.f;
    out_lp_ = 0.f;
    env_    = 0.f;
    lim_env_ = 0.f;
    ...
    lim_atk_c_ = 1.f - expf(-1.f / (ignis_k::LIMIT_ATK_MS * 0.001f * sr_));
    lim_rel_c_ = 1.f - expf(-1.f / (ignis_k::LIMIT_REL_MS * 0.001f * sr_));
    RecomputeVoices();
  }
  ...
  void Controls(const ControlSurface& cs,
    ...
    Smooth(asym_,      Mapf(k4, ignis_k::ASYM_MIN, ignis_k::ASYM_MAX));
    Smooth(structure_, k3);                     // 0..1, meaning per core
    // Register: bipolar +/- REGISTER_OCT octaves, continuous, no stacking (S1).
    Smooth(register_oct_, (k1 * 2.f - 1.f) * ignis_k::REGISTER_OCT);

    // Damping (K2) -> target T60 (log range). Recompute coeffs from it.
    t60_ = ExpMap(k2, ignis_k::T60_MIN_S, ignis_k::T60_MAX_S);

    // K5 bipolar attack/release of the output filter env.
    const float atk_ms = Mapf(k5, ignis_k::ENV_ATK_FAST_MS, ignis_k::ENV_ATK_SLOW_MS);
    const float rel_ms = Mapf(k5, ignis_k::ENV_REL_SLOW_MS, ignis_k::ENV_REL_FAST_MS);
 ...
 private:
  ...
  struct CombVoice {
    ...
    float  ap_x[ignis_k::DISP_STAGES] = {};
    float  ap_y[ignis_k::DISP_STAGES] = {};

    void Init(float* b) {
      buf = b;
      for (int i = 0; i < ignis_k::COMB_MAX_SAMPLES; i++) buf[i] = 0.f;
      wp = 0;
      for (int s = 0; s < ignis_k::DISP_STAGES; s++) { ap_x[s] = 0.f; ap_y[s] = 0.f; }
    }
    ...
    inline float Read(float d) const {
      ...
      i0 %= ignis_k::COMB_MAX_SAMPLES; if (i0 < 0) i0 += ignis_k::COMB_MAX_SAMPLES;
      i1 %= ignis_k::COMB_MAX_SAMPLES; if (i1 < 0) i1 += ignis_k::COMB_MAX_SAMPLES;
    ...
    inline float Process(float x, float ap_a) {
      ...
      for (int s = 0; s < ignis_k::DISP_STAGES; s++) {
        const float in = lp;
      ...
      buf[wp] = y;
      wp++; if (wp >= ignis_k::COMB_MAX_SAMPLES) wp = 0;
      return y;
    }
  };
  ...
  struct Modal {
    ...
    } m[ignis_k::MODES_PER_VOICE];

    void Reset() {
      for (int k = 0; k < ignis_k::MODES_PER_VOICE; k++) { m[k].y1 = 0.f; m[k].y2 = 0.f; }
    }
    inline float Process(float x) {
      float y = 0.f;
      for (int k = 0; k < ignis_k::MODES_PER_VOICE; k++) y += m[k].Process(x);
      return y;
    }
  };
  ...
  void RecomputeVoices() {
    ...
    ap_coeff_ = structure_ * ignis_k::DISP_MAX;

    for (int v = 0; v < ignis_k::ACTIVE_VOICES; v++) {
      const float midi = ignis_k::CHORD_ROOT_MIDI +
                         ignis_k::CHORD_INTERVALS_SEMI[v];
      ...
      float dtot = sr_ / f;
      const float maxd = (float)(ignis_k::COMB_MAX_SAMPLES - 3);
      ...
      if (g > ignis_k::G_MAX) g = ignis_k::G_MAX;
      combs_[v].g = g;

      // --- Modal voice: MODES_PER_VOICE partials, structure spreads them. ---
      const float spread = structure_ * ignis_k::MODAL_SPREAD_MAX;
      for (int k = 0; k < ignis_k::MODES_PER_VOICE; k++) {
        float ratio = ignis_k::MODAL_BASE_RATIOS[k] *
        ...
        float t60k = t60_ / powf(ratio, ignis_k::MODAL_DAMP_EXP);
        float r = powf(10.f, -3.f / (t60k * sr_));         // pole radius
        if (r > ignis_k::R_MAX) r = ignis_k::R_MAX;
  ...
  static inline void Smooth(float& s, float target) {
    s += ignis_k::PARAM_SMOOTH * (target - s);
  }

  inline float DcBlock(float x) {
    // y[n] = x[n] - x[n-1] + R*y[n-1]
    const float y = x - dc_x1_ + ignis_k::DC_BLOCK_R * dc_y1_;
    dc_x1_ = x; dc_y1_ = y;
    return y;
  }
  ...
  inline float Limit(float x) {
    ...
    float gr = 1.f;
    if (lim_env_ > ignis_k::LIMIT_THR) {
      const float over = lim_env_ - ignis_k::LIMIT_THR;
      gr = ignis_k::LIMIT_THR /
           (ignis_k::LIMIT_THR + over * ignis_k::LIMIT_RATIO_INV);
    }
    return x * gr;
  }
  ...
  CombVoice combs_[ignis_k::MAX_VOICES];
  Modal     modals_[ignis_k::MAX_VOICES];
};
