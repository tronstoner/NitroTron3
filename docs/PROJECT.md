...
A multi-mode digital effects pedal for bass guitar, built on the Electro-Smith Daisy Seed. Three independent effect modes are selectable via Switch 3. Presets are global across modes — each preset slot carries its own mode and full parameter state. Slots are organised into 3 banks of 8 (24 reachable presets, storage sized for up to 6 banks). Only the active mode runs at any time — inactive modes have zero CPU cost. Voiced for bass by default; `make INSTRUMENT=guitar` builds a guitar-voiced firmware variant (see "Instrument profile" in Current Status).
...
**QSPI migration — DONE.** The firmware no longer targets the 128 KB internal flash. Since `f170770` the Makefile sets `APP_TYPE = BOOT_SRAM` (linker script `STM32H750IB_sram.lds`): the internal flash holds only the Daisy bootloader, the app image is stored in the 64 MB QSPI flash, and the bootloader copies it into the 480 KB AXI SRAM at boot — so all code executes from on-chip RAM at full speed. The binding size limit is therefore the **480 KB SRAM region**, not flash. One-time setup per Seed is `make program-boot`; after that `make program-dfu` flashes the app through the bootloader.
...
Default timing (source of truth: `docs/ux-demo.html`, mirrored in `pedals/nitrotron3/constants.h`):
...
- **Internal balance:** Per-mode, calibrated by ear and committed as compile-time constants in `pedals/nitrotron3/constants.h`.
...
- **Tuning mode** — DROPPED. Originally planned as an in-pedal page-based tuning UI; in practice ear-tuning against `pedals/nitrotron3/constants.h` (with optional serial readouts when wanted) has been enough.
...
### Deferred
- **Mode B direct-texture mode** — DONE. K2 noon bypasses grain engine, K3 = micro-stutter.
...
### Phase 2 — Hardware
Port to Daisy. Final tuning happens by ear against real hardware and bass, with values committed back into `pedals/nitrotron3/constants.h`.
...
Complete index of everything under `docs/` (plus the two root-level entry
points). Each entry is marked **current** (trust it as a reference) or
**historical** (kept for context; the code has moved on — the doc's own header
...
- `ARCHITECTURE.md` — **current.** Multi-pedal platform layout: `core/` blocks vs `pedals/<name>/` bundles, the `PEDAL=` selector, the block/module/mode/shell vocabulary, and the Module-interface migration.
- `DSP_INVENTORY.md` — **current.** Measured inventory of every implemented DSP block: code size, CPU cost (host benchmark + M7 instruction counts + libm census), memory footprint, and per-mode budget estimates.
- `ROADMAP_V1.md` — **current.** Collected ideas for the road to v1, sorted into standalone increments.

### Mode references (as-built)

- `MODE_A_DRONE.md` — **partly historical.** Original Mode A (Bordun) spec. The oscillator / ladder / envelope DSP detail is still accurate; the *controls* have moved on — see `MODE_A_DISCOVERY.md` for the as-built K4/K5/tracking design.
- `MODE_A_DISCOVERY.md` — **current.** As-built design record for the Mode A K5 voice engine, bipolar K4 filter, and octave-locked tracking.
- `MODE_B_GRANULAR.md` — **current.** Mode B (Sprawl) spec, reflecting shipped behaviour: rolling buffer, grain scheduler, texture shaper, feedback bus, controls.
- `MODE_B_TEXTURE_IDEAS.md` — **current.** Design of the SW1=MIDDLE event-driven digital glitch, plus texture ideas not yet built.
- `MODE_C_POG_DISCOVERY.md` — **current.** The ERB-PS2 POG octave engine in SW1=MID / K4-CCW: method, references, tuning.
- `MODE_C_DISCOVERY.md` — **historical.** Mode C (Schism) original discovery notes. The SW1 drive → SW2 filter → mix architecture shipped, but SW2=DOWN is no longer Plague Bearer and many values shifted by ear.
- `PITCH_TRACKING.md` — **current.** YIN tracker: research, algorithm comparison, signal conditioning, the `TRACK_*` bass/guitar instrument profiles.

### Preset system

- `PRESET_IMPL.md` — **current.** As-built reference for the preset system.
- `PRESET_GLOBAL_PLAN.md` — **historical.** The per-mode → global+banks migration plan, with notes on where the implementation diverged.
- `PROJECT.md` § Preset System — **current.** The behavioural spec (above in this file).

### Firmware distribution

- `WEB_UPDATER.md` — **current.** Browser-based firmware updater at `docs/updater/`: architecture, publishing, release-process addition.
- `USER_MANUAL.md` / `USER_MANUAL.pdf` — **current.** End-user manual. The PDF is generated by `make manual` (pandoc + weasyprint); do not hand-edit it.

### Historical / superseded

Kept deliberately — they record why something was tried and dropped. Do not read
any of these as current behaviour.

- `MODE_B_DISCOVERY.md` — bipolar K2/K3 redesign plan; shipped with divergences.
- `MODE_B_IMPL.md` — original Mode B staging plan, largely superseded.
- `MODE_B_REVERB_TASK.md` — completed task doc for the Clouds wet-path reverb.
- `MODE_C_PHASER_PLAN.md` — the 3-band BPF plan for SW2=DOWN; as-built is a 6-stage allpass phaser instead.
- `plague-bearer-daisy-handover.md` — Plague Bearer filter emulation handover; modelled, never kept.
- `MULTI_DIP_POSTMORTEM.md` — multi-dip YIN pseudo-polyphonic tracking, abandoned 2026-07-21. Establishes the "no per-note poly tracking" boundary.
- `STUTTER_SPEC.md` — behavioural spec for the retired `StutterVoice` engine.
- `STUTTER_IMPLEMENTATION_NOTES.md` — crackle-free stutter debugging notes from the same engine. Still useful reading on click-free windowing.
...
- `ChronoTron3/impulse resonator - armitage/IMPULSE_SYNTH_SPEC.md` — working spec (design intent) for the impulse-resonator synth module (*armitage*, archived), with `saturation.py` / `validate.py` alongside it.
- `ChronoTron3/impulse resonator - armitage/ARMITAGE_AS_BUILT.md` — **as-built reference for *armitage*** (archived module; still accurate) (read first for anything touching note detection / triggering / voicing): the chord-detection pipeline (filterbank → onset → settle → snapshot → sub-harmonic fundamental filter), HPF onset detection (separate from the gate), the excitation-envelope guardrail, nearest-note voice-leading + linear fixed-time portamento (K4), audio-rate + per-voice declicking, and the resonator/FM/drive voice — plus a "directions parked" section (auto-voicing dead-end, pitch-shift/POG path for density). Explains the model + the hard-won gotchas; values live in `armitage_constants.h`.
...
Not prose — but they are source of truth for things the docs describe.

- `ux-demo.html` — **source of truth** for the preset-system LED/footswitch timing constants mirrored in `pedals/nitrotron3/constants.h`.
- `mode-b-engines.html` — Mode B audio-engine prototype/demo.
- `gen_layout_svg.py` — generates the per-mode pedal-layout SVGs in `assets/` used by the manual. Never hand-edit the SVGs.
- `gen_icons.py` — generates the knob-direction icons in `assets/` used by the manual.
- `manual.css` — stylesheet for the PDF manual build.
- `updater/` — the web firmware updater application itself (see `WEB_UPDATER.md`).
