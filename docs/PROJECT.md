...
- **Mode C (Schism)** — IN PROGRESS. C.1 + C.2 DONE: K5 wet level + K6 equal-power mix; sine wavefolder under SW1=UP with K4 fold amount, internal compensation via `SINEFOLD_COMP_AT_MAX`. SW2 (filter) still unwired; K1–K3 unused.
- **Next** — Mode C C.3: Moog ladder under SW2=UP with K1 cutoff, K2 resonance, K3 env-to-cutoff (bipolar, deadzone). FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
