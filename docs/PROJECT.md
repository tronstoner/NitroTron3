...
- **Mode C (Schism)** — IN PROGRESS. C.1–C.3 DONE: K5 wet level + K6 equal-power mix; sine wavefolder under SW1=UP (K4 fold); Moog ladder under SW2=UP (K1 cutoff 80 Hz–8 kHz exp, K2 resonance up to 0.95 ceiling, K3 bipolar env-to-cutoff with ±5% deadzone, ±4 octaves at K3=±1). `MoogLadder` gained opt-in `SetResonance()` with half-sample-compensated Huovilainen feedback; Mode A unaffected (res defaults to 0). Separate `ladder_c` instance prevents filter-state leak across mode switches.
- **Next** — Mode C C.4: Plague filter (`SvfNonlinear` + `Plague` classes) under SW2=DOWN. FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
