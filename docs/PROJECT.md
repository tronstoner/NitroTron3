...
- **Mode C (Schism)** — IN PROGRESS. C.1–C.5 DONE: K5 repurposed to universal filter drive (1× → 8×, ladder drive for Moog / pre-tanh saturator for Grendel & Plague) + K6 equal-power mix; sine wavefolder under SW1=UP (K4 fold); Moog ladder under SW2=UP (K1 cutoff, K2 sqrt-curved resonance up to self-osc, K3 bipolar env-to-cutoff with passive-bass ×10 normalization and 10 kHz top clamp); Plague filter under SW2=DOWN (`svf_nonlinear.h` + `plague.h`) — Plague tuning still needs another pass; Grendel formant filter under SW2=MID — 4 parallel RBJ BPF biquads, vowel path ee→eh→ah→oh→oo, K1 path, K2 size (mouth scale), K3 bipolar env on path (block-rate coef updates to avoid cosf/sinf per sample). New `grendel.h`. Vowel table and gains ear-tuned in C.5.
- **FLASH at 89.06%** — dipped under the QSPI-migration threshold after swapping out the chorale formant filter (15 biquads of state + per-block coef compute). Remaining Mode C work (C.6 SW1=MID drive flavor, possible loudness-compensation curves) may still push back over 90% — QSPI migration ready when needed via one-line linker swap to `STM32H750IB_qspi.lds`.
- **Next** — Mode C C.6: lock K3 polarity for SW2=UP/MID (bipolar vs unipolar) from listening; redo Plague tuning (drive/character was failing hard in C.5 listening); implement SW1=MID drive flavor. All of this is gated on pedal access. FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
