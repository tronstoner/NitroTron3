...
- **Mode B (Sprawl)** — IN PROGRESS. Granular engine working: ring buffer (8 s SDRAM), grain scheduler, 8 voices, Hann windowing, pitch-tracked harmony (fixed interval + resonance modes), texture shaper (decimator/wavefolder bipolar on SW1 UP, chorale formant filter on MID, ringmod on DOWN), wet HPF 150 Hz, equal-power mix. Knob layout: K1=interval, K2=buffer range, K3=character/glitch (merged, splittable), K4=texture amount, **K5=reverb/feedback bipolar** (CCW=Clouds reverb, center=off, CW=feedback 0.95 ceiling), K6=mix. Direct-texture mode (K2 fully CCW): grain engine bypassed, input→texture shaper, K3=micro-stutter. **Wet-path reverb**: Clouds FDN (vendored from MI, MIT), 32 kHz internal via polyphase 48↔32 resampler, mono in / stereo out (collapsed to mono at mix for future stereo upgrade), reverb tail does not feed ring buffer.
- **Mode C (Schism)** — IN PROGRESS. C.1–C.5 DONE: K5 wet level + K6 equal-power mix; sine wavefolder under SW1=UP (K4 fold); Moog ladder under SW2=UP (K1 cutoff, K2 resonance, K3 bipolar env-to-cutoff); Plague filter under SW2=DOWN (`svf_nonlinear.h` + `plague.h`); Grendel formant filter under SW2=MID — 4 parallel RBJ BPF biquads, vowel path ee→eh→ah→oh→oo, K1 path, K2 size (mouth scale), K3 bipolar env on path (block-rate coef updates to avoid cosf/sinf per sample). New `grendel.h`. Initial vowel table and gains are first-pass; ear-tune all of C.3/C.4/C.5 when hardware is back.
- **FLASH at 90.01%** — at the threshold from PROJECT.md's QSPI-migration plan. Remaining work for Mode C (C.6 SW1=MID drive flavor, possible loudness-compensation curves) will likely require switching the linker script to `STM32H750IB_qspi.lds`.
- **Next** — Mode C C.6: lock K3 polarity for SW2=UP/MID (bipolar vs unipolar) from listening; decide K5's fate (keep or drop); implement SW1=MID drive flavor. All of this is gated on pedal access. FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
