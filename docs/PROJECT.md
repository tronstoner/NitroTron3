...
- **Mode C (Schism)** — IN PROGRESS. C.1–C.5 DONE: K5 is now a bipolar pre-filter drive (CCW attenuate ~−12 dB, noon unity, CW up to 8× — universal across all SW2 modes); sine wavefolder under SW1=UP (K4 fold); Moog ladder under SW2=UP using new `MoogLadderV2` (single input saturator post-feedback, no per-stage tanh darkening; k-cutoff cross-comp for consistent self-osc; (1+res) input level comp; +0.05 asymmetric DC bias for transistor-flavored even harmonics) — K1 cutoff, K2 sqrt-curved resonance up to self-osc, K3 bipolar env-to-cutoff with passive-bass ×10 normalization and 10 kHz top clamp; Plague filter under SW2=DOWN (`svf_nonlinear.h` + `plague.h`) — still needs another tuning pass; Grendel formant filter under SW2=MID — 4 parallel RBJ BPF biquads, vowel path ee→eh→ah→oh→oo, K1 path, K2 size (mouth scale), K3 bipolar env on path (block-rate coef updates). Mode A's `MoogLadder` is intentionally untouched (signed off). Per-filter input pads (×0.3 for Moog and Grendel; Plague unpadded) put spectrum-shaping filters in their clean sweet zone at K5 noon while preserving Plague's hot-input fold zone. Post-filter signal chain: `PeakLimiter` with 2-band split at 160 Hz (LF preserved so bass fundamentals don't duck under HF resonance peaks; HF soft-knee limited with "warmth-when-working" tanh blend that only blooms while the limiter is reducing gain), then an amp-env VCA (`wet *= env_val * 12`) that gates the wet path so self-resonance doesn't ring alone when the bass isn't playing.
- **FLASH at 90.40%** — modestly back above the QSPI-migration threshold after adding MoogLadderV2, PeakLimiter, and the amp-env VCA. Migration still ready via one-line linker swap to `STM32H750IB_qspi.lds` if Plague re-tuning or other Mode C work pushes higher.
- **Next** — Mode C C.6: redo Plague tuning (drive/character/balance — user has separate research coming, see `docs/plague-bearer-daisy-handover.md`); implement SW1=MID drive flavor; lock K3 polarity for SW2=UP/MID if any drift surfaces. All gated on pedal access. FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
