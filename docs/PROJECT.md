...
- **Mode B (Sprawl)** — IN PROGRESS. Granular engine working: ring buffer (8 s SDRAM), grain scheduler, 8 voices, Hann windowing, pitch-tracked harmony (fixed interval + resonance modes), texture shaper (decimator/wavefolder bipolar on SW1 UP, clean on MID, ringmod on DOWN), wet HPF 150 Hz, equal-power mix. Knob layout: K1=interval, K2=buffer range, K3=character/glitch (merged, splittable), K4=texture amount, **K5=reverb/feedback bipolar** (CCW=Clouds reverb, center=off, CW=feedback 0.95 ceiling), K6=mix. Direct-texture mode (K2 fully CCW): grain engine bypassed, input→texture shaper, K3=micro-stutter. **Wet-path reverb**: Clouds FDN (vendored from MI, MIT), 32 kHz internal via polyphase 48↔32 resampler, mono in / stereo out (collapsed to mono at mix for future stereo upgrade), reverb tail does not feed ring buffer.
