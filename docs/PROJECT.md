...
- **Mode C (Schism)** — IN PROGRESS. C.1–C.4 DONE: K5 wet level + K6 equal-power mix; sine wavefolder under SW1=UP (K4 fold); Moog ladder under SW2=UP (K1 cutoff, K2 resonance, K3 bipolar env-to-cutoff); Plague filter under SW2=DOWN — two parallel saturating SVFs at fixed corners (220 Hz / 1.8 kHz) with per-integrator tanh, constant-sum K1 input balance, tandem K2 (input gain + feedback drive), bipolar K3 env on balance. New `svf_nonlinear.h` (Cytomic/Simper form) and `plague.h`. Initial constants from `MODE_C_DISCOVERY.md`; loudness compensation curve still flat (1.0) — ear-tune in C.4 verification.
- **Next** — Mode C C.5: Grendel formant filter (SW2=MID, vowel-path interpolator). All C.3/C.4/C.5 ear-tuning blocked on pedal access. FS1+FS2 dual-hold bootloader (Phase 2) and pitch tracking improvements still deferred.
