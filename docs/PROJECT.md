...
- `ChronoTron3/impulse resonator - armitage/ARMITAGE_AS_BUILT.md` — **as-built reference for *armitage*** (read first for anything touching note detection / triggering / voicing): the chord-detection pipeline (filterbank → onset → settle → snapshot → sub-harmonic fundamental filter), HPF onset detection (separate from the gate), the excitation-envelope guardrail, nearest-note voice-leading + fixed-time portamento, and the resonator/FM/K4-drive voice. Explains the model + the hard-won gotchas; values live in `armitage_constants.h`.
