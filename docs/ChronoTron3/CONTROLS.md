...
| KNOB 5 | Loop fade in/out | CCW = instant → CW = max. **Linear-in-dB** fade (constant dB/sec), in and out are inverse ratios ⇒ **perceptually symmetric** (not just visually). Durations `ATTACK_MAX_S` / `RELEASE_MAX_S` (default 6 s : 6 s, equal ⇒ symmetric), over a ~60 dB span (`FADE_FLOOR`). Applied on loop start / stop / mute |
