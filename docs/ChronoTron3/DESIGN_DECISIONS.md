...
| G3 | **The tone filter and its narrow-band makeup gain are the USER'S EQ (mnemonic K1 tilt + K3 narrow — K4/K5 before the 2026-09-14 remap). Do not alter them to solve an unrelated problem** (e.g. a hot self-oscillation). Fix such problems on a *separate* wet-only stage. | `96c360c` + the wet safety roll-off constant block: the piercing high-pitched self-osc (narrow + makeup boost) is tamed by a dedicated de-esser on the wet output "WITHOUT touching the EQ, the makeup, or the feedback loop." |
...
| G6 | **In-loop vs out-of-loop placement is deliberate per stage** — don't move a stage across the feedback boundary casually. The tone filter (K1/K3), tape drive, and the degrade engine (K4) all live **IN** the loop, so repeats age through them (`Filter(x)` runs before `delay_.Write` in both paths). Freeze sums **outside** the loop (doesn't recirculate). The feedback tap and wet output both read the buffer (`ds`/`dd`), which is already filtered+coloured from prior laps. | Verified in current code (`mnemonic.h`). The K5-CCW reverb sums **outside** the loop for the same reason freeze does. History: filters were briefly moved OUT during the feedback-drown investigation, then returned IN once the drown was traced to the Mode-B ducker (`605368d`). |
...
| Decision | Context / problem | Why this over alternatives | Alternatives rejected (why) | Status |
|---|---|---|---|---|
| **Remove the FAST-attack ducker; later RE-ADD it slow-attack** | The ported ducker had a 40 ms attack → it ducked normal repeats and K1 varispeed sweeps (read tap moving into louder regions) = the "feedback-drown" regression | Removing it fixed the drown, but left no sustained-level cap → self-osc later ran ear-piercing. The real fix was the ATTACK TIME: a 500 ms slow attack (Sprawl-proven) caps only sustained build-up, not transients/sweeps | Keep the fast ducker (drown); no ducker at all (self-osc screams) | **Superseded** — fast ducker removed (`605368d`), slow-attack ducker re-added (`MNEM_FB_DUCK_*`). See feedback ducker row |
...
| **Feedback build-up ducker (Sprawl-proven, SLOW attack)** — the loop level control | Self-oscillation was ear-piercingly loud; a wet-output de-esser (below) did nothing audible and was the wrong layer | A slow-attack (500 ms) envelope on the loop read drives a 1:inf attenuation of the feedback GAIN once the SUSTAINED level exceeds `MNEM_FB_DUCK_THR` — runaway is capped to a controlled simmer/drone at any pitch. Slow attack = transients, repeats, K1 sweeps all pass; only sustained build-up is caught. Copied Sprawl's structure (which never has feedback-loudness issues even with a feedbacking pitch shifter) | Wet-output de-esser (band-aid on the wrong layer, inaudible); the earlier FAST-attack ducker (caused the "drown" — it reacted to transients/sweeps) | In force (`MNEM_FB_DUCK_*`). **Lesson: the ducker was removed in `605368d` for the wrong reason — the problem was the FAST attack, not the ducker itself.** |
| **Zipper fix: audio-rate (~5 ms) smoothing of feedback, degrade drive, filter cutoffs + makeup** | Control tick is ~10 ms; stepped params zipper | SVF coeffs recompute per-sample from smoothed cutoffs (2 tanf via `CopyCoefFrom`); delay time already smoothed by the glide | (unsmoothed) | In force (`2774895`, `MNEM_SMOOTH_MS`). **Flagged not-smoothed:** BBD decimation params (degrade CCW, inherently steppy) |

**K5 bipolar — reverb (CCW) / feedback (CW), 2026-09-14.**

| Decision | Context / problem | Why this over alternatives | Alternatives rejected (why) | Status |
|---|---|---|---|---|
| **The CCW reverb side is NOT feedback-free** (unlike sprawl, whose CCW kills feedback entirely) | Copying sprawl literally gave one repeat washing into reverb — a delay at zero feedback has nothing under the wash. Sprawl gets away with it because its grain engine self-sustains | CCW mirrors the CW feedback ramp, so the two halves read the same, and only the top is clamped | Strict mutual exclusivity (musically empty on a delay); reverb *inside* the loop (G6 — runaway risk, and it only makes sense if CCW keeps feedback) | In force |
| **`MNEM_REVERB_FB_CEIL` is a CLAMP on the ramp, never a multiplier** | "Cap the top end so it doesn't self-oscillate" was implemented as a scale on the whole ramp | A clamp flattens the top and leaves every position below it identical to the CW side, which is what a cap means | Scaling the ramp — it weakened *every* position, put the far end below a setting already judged too weak, and the repeats vanished. Cost several flash cycles and read as a bug | In force — do not re-derive |
| **Reverb LEVEL is `MNEM_REVERB_IN_GAIN`, not the blend** | "Reverb too quiet" was chased through the blend first | The blend is a crossfade: it only decides how much reverb *replaces* the direct delay, so pushing it eats the repeats without ever getting louder. Input gain sets the level | Raising the blend ceiling toward 1.0 (removes the repeats entirely at the extreme — the same crossfade trap) | In force; bracketed by ear to 0.50 |
| **Blend ceiling < 1.0** (`MNEM_REVERB_AMT_MAX`) | At 1.0 the extreme is fully wet, so the CCW feedback is crossfaded away exactly as it grows — the two ramps fight | Capping keeps direct delay present at the extreme | 1.0 (sprawl's value; fine there, not here) | In force |

**Lesson — the feedback number is not the loop gain.** Each lap is
`fb_gain × FbCtl × FbDuck × (filter makeup, always-on 1.4 tape drive, degrade
colour)`, and **FbCtl is level-dependent by design**: it scales feedback *down*
as the loop quietens, to accelerate tails to silence. So the two failure modes
look contradictory — a nominal 0.98 self-oscillates (loud loop, FbCtl ≈ 1, drive
over unity) while a uniformly scaled 0.70 dies after one repeat (quiet tail
scaled to ~0.49). Tune anything in this loop **by ear**; arithmetic against 1.0
is meaningless.

### Tone filter (K1 tilt / K3 narrow)

| Decision | Context / problem | Why this over alternatives | Alternatives rejected (why) | Status |
|---|---|---|---|---|
| **The tone filters live IN the feedback loop** (repeats age through the tone filter; feedback + wet both tap the already-filtered buffer read) | Aging-tape character: each repeat should re-colour through the EQ | Matches the classic aging-tape default in the concept doc; the drown that once seemed filter-caused was actually the Mode-B ducker | Filters OUT of the loop — tried briefly during the drown investigation (`605368d`) then **reverted** once the drown was traced to the ducker | In force (verified in code: `Filter(x)` before `delay_.Write`) |
...
### Degradation engine (K4) — see `mnemonic-degradation-colour-spec.md` for the full spec
...
| **Base tape drive always on** (`MNEM_TAPE_DRIVE`), independent of the degrade knob | Brief: degrade is "which flavour of lo-fi", not an on/off gate | An always-on warmth means the tone filter voices into a base tape drive; degrade layers extra colour | degrade as the only source of colour (would make noon fully clean/sterile) | In force (concept §K3; `b08bb42`) |
...
| **Edge primary = bit-identical to SW2-MID** (delay = quarter × the division knob, full tone filter + drive + degrade) | Want Edge to *add* one thing, not be a separate voicing | Primary stays the known-good colored line; Edge "literally adds ONE thing" — a secondary line | A distinct primary voicing (would fork behaviour) | In force (`489bf02`) |
...
| **Edge secondary voice = CLEAN telephone band-pass** (350 Hz–2.5 kHz, ~25% tone-filter follow), no drive/shaper | See the secondary-line journey below — this is the endpoint | The mid band alone gives the separation from the primary; no shaper needed; light tone-filter follow keeps tonal cohesion | Octave-up, Chebyshev drive, RMS-normalize (all rejected — see next section) | In force (`7d8cf14`) — `MNEM_SEC_DRIVE_ENABLE` kept as a false flag to re-enable the grit for A/B |
...
| **FAST-attack (40 ms) build-up ducker** | The "feedback-drown": a fast attack reacted to normal repeats and K1 varispeed sweeps, slamming feedback down | The *ducker* was right, the *attack time* was wrong. Re-added with Sprawl's 500 ms slow attack, it catches only sustained runaway and never the transients/sweeps. Lesson: don't discard a mechanism for a tuning fault |
| **Wet-output HF de-esser** | Tried to tame the piercing self-osc on the output; inaudible (loop tanh already bounds `delayed` to ~0.7, so a de-esser only shaved ~1.5 dB) and it was the wrong layer | Level control belongs in the FEEDBACK path (cap the loop), not on the output. Superseded by the slow-attack ducker |
...
clean/dry signal and the tone EQ are sacrosanct; pedal levels are relative to
