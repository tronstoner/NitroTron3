...
// (Stutter engine removed — grain scheduler handles low-K2 sparse character
// via gap inflation, K3-gated emission, complement crossfade against dry,
// and pitch-aware delay sizing so reads stay inside valid buffer content.)
...
void ProcessGranular(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out,
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (grain_timer <= 0 && !emissions_off) {
      ...
      // Pitch-aware delay: forward grain at rate R consumes L×R input samples
      // for L output samples, so start must be at least L×R behind the write
      // head to avoid running into stale buffer content. Reverse grains only
      // need start ≤ write head, i.e. delay ≥ L.
    ...
    // Wet HPF disabled for evaluation. When re-enabled, scale by (1-sparsity)
